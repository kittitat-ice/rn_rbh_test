...
const DoingList = props => {
  ...
  const {doingList} = useSelector(selectTaskReducer);
  ...
  const getCurrentOffset = () => {
    return Math.ceil(doingList.length / limit);
  };

  // api request
  const requestDoing = async (offset = 0, replaceState) => {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    dispatch(clearDoingList());

    // api load too fast
    // cause redux state to duplicate
    setTimeout(async () => {
      await requestDoing(0, true);
      setRefreshing(false);
    }, 200);
  }, [doingList]);
  ...
  const deleteItem = id => {
    const newList = doingList.filter((item, index) => item.id !== id);
    dispatch(setDoingList(newList));
  };
  ...
  const renderItem = ({item, index}) => {
    const sameDay =
      index === 0
        ? false
        : isSameDay(
            new Date(doingList[index].createdAt),
            new Date(doingList[index - 1].createdAt),
          );
      ...
      <View style={styles.listContainer}>
        ...
        <Swipeable
          ...
          <View style={styles.rowContainer}>
            <View style={styles.iconDoingPlaceholder} />
