...
import {setSavedPin} from '../../redux/slices/authSlice';
...
const PinSetting = props => {
  ...
  useEffect(() => {
    if (pin.length === pinCount) {
      if (!isRepeatPin) {
        // CASE: initial set pin
        setCheckPin(pin);
        setPin('');
        setIsRepeatPin(true);
        setInstructionText(t('pin_again'));
      } else {
        // CASE: repeat same pin
        if (pin === checkPin) {
          dispatch(setSavedPin(checkPin));
          props.navigation.replace('PinLock');
        } else {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        <View style={styles.topContainer}>
          <Text style={styles.topText}>{t('pin_setting')}</Text>
        </View>
          ...
          <View style={styles.rowContainer}>
            {isRepeatPin ? (
              <TouchableOpacity
                ...
                style={[styles.pinButton]}>
                <Text style={[styles.pinButtonText, {fontSize: 16}]}>
                  {'Reset'}
                </Text>
              </TouchableOpacity>
            ) : (
